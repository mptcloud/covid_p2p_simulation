...
class ContactBook:
    ...
    def get_symptomatic_contacts_counts(
            self,
            humans_map: typing.Dict[str, "Human"],
            max_order: int = 1,
            curr_order: int = 0,
        ...
        """Traces and returns the number of Nth-order contacts that have reported symptoms."""
        if curr_order == 0:
            for human in humans_map.values():
                human.contact_book._is_being_traced = False
            self._is_being_traced = True
        ...
        for contact in self.get_contacts(humans_map, only_with_initial_update=True):
            ...
            if not contact.contact_book._is_being_traced and contact.has_logged_symptoms and \
                    ...
                    (contact.env.timestamp -
                     contact.last_date["reported_symptoms"]) < self.tracing_n_days_history:
                count_map[curr_order + 1] += 1
                contact.contact_book._is_being_traced = True
            if max_order > curr_order + 1:
                contact.contact_book.get_symptomatic_contacts_counts(
                    humans_map=humans_map,
                    max_order=max_order,
                    curr_order=curr_order + 1,
                    count_map=count_map,
    ...
    def get_risk_level_update_counts(
            self,
            humans_map: typing.Dict[str, "Human"],
            max_order: int = 1,
            curr_order: int = 0,
        ...
        """Traces and returns the number of risk level updates sent by Nth-order contacts."""
        if curr_order == 0:
            for human in humans_map.values():
                human.contact_book._is_being_traced = False
            self._is_being_traced = True
        ...
        for contact in self.get_contacts(humans_map, only_with_initial_update=True):
            ...
            if not contact.contact_book._is_being_traced:
                ...
                contact.contact_book._is_being_traced = True
            if max_order > curr_order + 1:
                contact.contact_book.get_risk_level_update_counts(
                    humans_map=humans_map,
                    max_order=max_order,
                    curr_order=curr_order + 1,
                    count_map=count_map,
    ...
    def get_risk_level_update_count(self):
        """Returns the number of risk level updates sent by this contact book's owner."""
        count = 0
        for day, msgs in self.encounters_by_day.items():
            for msg in msgs:
